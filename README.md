# TIA_Add_In_DeviceChange_Weidmueller
This plugin can implement batch change of UR20 BASIC configuration upgrade to HW2 in the TIA Portal

此插件可以实现在TIA Portal中批量更改UR20 BASIC组态升级为HW2。
...
1. Select the ***UR20 BASIC*** in the DevicesAndNetworks.  
在硬件与网络编辑器中选择***UR20 BASIC设备***

2. Open the shortcut menu.  
打开快捷菜单。

3. In the shortcut menu under the "Add-Ins" entry, execute the command **"UR20_BASIC" -> "Change device / version"**.  
在快捷菜单的“插件”(Add-Ins) 条目中，执行命令 **"UR20_BASIC" -> "Change device / version"**。
